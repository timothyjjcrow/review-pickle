...
def home():
    """Home page route"""
    try:
        return render_template('index.html', reviews=SAMPLE_REVIEWS)
    except UnicodeDecodeError as e:
        logger.error(f"Template encoding error: {str(e)}")
...
def reviews_list():
    ...
    try:
        return render_template('reviews/list.html', reviews=SAMPLE_REVIEWS)
    except Exception as e:
        logger.error(f"Error rendering reviews list: {str(e)}")
        return f"An error occurred: {str(e)}", 500
...
def api_reviews_list():
    """API endpoint for reviews listing"""
    try:
        # Convert datetime to string for JSON serialization
        reviews_copy = []
        ...
        return jsonify(reviews_copy)
    except Exception as e:
        logger.error(f"Error in API reviews list: {str(e)}")
...
if __name__ == '__main__':
    try:
        # Ensure template files are read with UTF-8 encoding
